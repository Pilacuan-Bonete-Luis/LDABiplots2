shiny::shinyServer(function(input,output,session) {
  ...
  observeEvent(input$getData, {
    ...
    values$datxl <- as.data.frame(
      unclass(datxl), stringsAsFactors = TRUE)
    datxl <- values$datxl %>% dplyr::group_by(News)
...
##---------------scraping--------
  observeEvent(input$runsearch, {
    ...
    for(i in 0:input$numberpag){
      ...
      Diario$Diario<- stringr::str_trim(Diario$Diario,side = "both")
      Diario$Diario<- stringr::str_to_lower(Diario$Diario,locale = "es")
      Diario$Diario<-chartr('áéíóúñ','aeioun',Diario$Diario)
      Noticia<-data.frame(Noticia=news_array)
      Noticia$Noticia<- stringr::str_to_lower(Noticia$Noticia,locale = "es")
      Noticia$Noticia<-chartr('áéíóúñ','aeioun',Noticia$Noticia)
      Data<-cbind(ID=row.names(Diario),Diario, Noticia)
      values$Data <- Data %>% dplyr::group_by(Noticia) %>% dplyr::filter (! duplicated(Noticia))
...
#tabla de noticas por diario
  ...
  ### Permitir que este disonible el nombre del periodico

  observe({
    req(values$Data)
    dat <- values$Data
    lista <- unique(dat$Diario)
    updateSelectizeInput(
      session = session,
      inputId = "selectnews",
      label = "Select/deselect all options",
      choices = as.character(lista),
      server = TRUE,
      selected = NULL
    )
  })
  ...
  observeEvent(input$dtm.update, {
    ...
    values$dtmt <- removeSparseTerms(CONVERT,
                                sparse= input$sparce)
     values$dtmF <-  chinese.misc::m3m(values$dtmt,to="dgCMatrix")
    ...
    #values$freq <- colSums2(values$dtmF)
  ...
  observeEvent(input$dtmexcel.update, {
    ...
    values$dtmFxl <-  chinese.misc::m3m(values$dtmtxl, to="dgCMatrix")
    #values$dtmFxl <- as(values$dtmtxl,"CsparseMatrix")
    Originalxl <- dim(odtm)
    Without_Sparsity <- dim(values$dtm)
    ...
    values$freq <- colSums(as.matrix(values$dtmFxl))  #
     #values$freq <- colSums2(values$dtmFxl)
    values$wf <- data.frame(word=names(values$freq), freq=values$freq)
    beepr::beep(2)
  ...
  output$plot_gg <-  highcharter::renderHighchart({
    export
    values$wf %>% dplyr::top_n(input$b, freq) %>%
      highcharter::hchart("column",
             highcharter::hcaes(x = word, y = freq),
             color = input$colorbar,
             borderColor = "black") %>%
      highcharter::hc_add_theme(highcharter::hc_theme_ggplot2()) %>%
      highcharter::hc_xAxis(title = list(text = "Term")) %>%
  ...
  output$plot_gf<-  highcharter::renderHighchart({
    export
    values$wf %>% dplyr::top_n(input$c, freq) %>%
      highcharter::hchart( "wordcloud",
              highcharter::hcaes(name = word, weight = freq)) %>%
      highcharter::hc_exporting(
        enabled = TRUE,
        formAttributes = list(target = "_blank"),
        buttons = list(contextButton = list(
  ...
  output$plotcoc <- renderPlot({
    ...
    values$plotco <- textplot::textplot_correlation_glasso(term_correlations,
                                                 exclude_zero = TRUE)
    values$plotco
  })
  ...
  output$plot_gi <-  highcharter::renderHighchart({
    ...
    export
    values$coherence_mat %>%
      highcharter::hchart("line", highcharter::hcaes(x = k, y = coherence)) %>%
      highcharter::hc_add_theme(highcharter::hc_theme_ggplot2())%>%
  ...
  output$summLDA <- DT::renderDT({
    ...
    DT::datatable(data = summar, extensions = 'Buttons',
                  options = list(dom = 'Bfrtip',
                                 ...
                                 lengthMenu = list(c(5, 25, 100, -1),
                                                   c('5', '25', '100','All'))))%>%
      DT::formatRound( columns= c("coherence","prevalence"),
                   digits=5)
  })
  ...
  output$phi <- DT::renderDT({
    ...
    tidy_beta <- data.frame(topic = as.integer(stringr::str_replace_all(rownames(values$model$phi),                                                                "t_", "")
    ),
  ...
  output$plot_worcloud <-  highcharter::renderHighchart({
    ...
    tidy_beta <- data.frame(
      topic = as.integer(stringr::str_replace_all(rownames(values$model$phi),
                                         "t_", "")),
  ...
  output$plot_heatmap <-  highcharter::renderHighchart({
    ...
    if(input$load == "import"){
      heatm <- values$tidy_theta %>%
        ...
        dplyr::summarise(proportion= mean(gamma))
      export
      heatm %>%
        highcharter::hchart("heatmap", highcharter::hcaes(x = Newspaper,
                                y = as.integer(stringr::str_replace_all(topic,"t_", " ")) ,
                                value =proportion)) %>%
        highcharter::hc_colorAxis(stops= colr,
                     min=min(heatm$proportion),
                     max= max(heatm$proportion)) %>%
        highcharter::hc_yAxis(title = list(text = "Topic"))%>%
        highcharter::hc_credits(
          enabled = TRUE, text = "Source: LDABiplots",
          style = list(fontSize = "12px")
        )%>%
      ...
      heatm <- values$tidy_theta %>%
        dplyr::group_by(topic,diario) %>%
        dplyr::summarise(proportion= mean(gamma))
      export
      heatm %>%
        highcharter::hchart("heatmap", highcharter::hcaes(x = diario,
                                y = as.integer(stringr::str_replace_all(topic,"t_", " ")) ,
                                value =proportion)) %>%
        highcharter::hc_colorAxis(stops= colr,
                     min=min(heatm$proportion),
 ...
 if (input$selectypebiplot == 'HJ_Biplot') {
  ...
  output$eigen <-  DT::renderDT({
    ...
    DT::datatable(data = dat,
      ...
      DT::formatRound( columns= c("dat.eigenvalues"),
                   digits=5)
  ...
  output$variance <-  DT::renderDT({
    ...
    DT::datatable(data = dat,
      ...
      DT::formatRound( columns= c("dat.explvar"),
                   digits= 2)
  ...
  observeEvent(input$Runpestel,{
    ...
    values$didybeta <- tidy_beta %>% spread(topic, beta)
    #names <- didybeta$term
    #didybeta <- as.data.frame(didybeta)
    #values$didybeta <- dplyr::select(didybeta,-term)
    #rownames(values$didybeta)<- names

    pestel <- system.file("extdata", "pestel.xlsx",
                package = "LDABiplots")
    values$pestel <- readxl::read_excel(pestel)
    if(input$language2 =="english"){
    word_phi <- as.data.frame(values$didybeta[,1])
    names(word_phi)[1]="Word"
    pestel_word <- word_phi %>%
      merge(values$pestel, ., by = "Word")
    values$Cluster_pestel <- dplyr::left_join(word_phi,pestel_word, by="Word")
    values$Cluster_pestel <- values$Cluster_pestel%>% dplyr::distinct(Word, .keep_all = T)
    } else if (input$language2 =="spanish"){
    word_phi <- as.data.frame(values$didybeta[,1])
    names(word_phi)[1]="Palabra"
    pestel_word <- word_phi %>%
      merge(values$pestel, ., by = "Palabra")
    values$Cluster_pestel <- dplyr::left_join(word_phi,pestel_word, by="Palabra")
    values$Cluster_pestel <- values$Cluster_pestel%>% dplyr::distinct(Palabra, .keep_all =
                                                                        )}

  })

  output$plotPestel<- renderPlot({
    req(values$didybeta)
    didybeta <- values$didybeta
    names <- didybeta$term
    didybeta <- as.data.frame(didybeta)
    didybeta <- dplyr::select(didybeta,-term)
    rownames(didybeta)<- names
    hj2 <- FactoMineR::PCA(didybeta)
    cluster <- values$Cluster_pestel
    plotpestel  <- factoextra::fviz_pca_biplot(hj2,title = "LDA_BIPLOT_PESTEL", axes = c(input$axisxpestel, input$axisypestel),habillage = as.factor(cluster$Puntuacion),
                                select.ind = list(cos2 = input$cosind),
                                select.var = list(cos2 = input$cosvar), repel = TRUE)
    values$plotpestel <- plotpestel+ themesb[[input$themepestel]]
    values$plotpestel + theme(legend.position = "bottom")+
      xlab(paste0("Dim_", input$axisxpestel)) + ylab(paste0("Dim_", input$axisypestel))
  })

  observe({
    if (isTRUE(is.null(values$plotpestel))) {
      shinyjs::disable("downpestel")
      shinyjs::disable("butdownpestel")
    }
    else {shinyjs::enable("downpestel")
      shinyjs::enable("butdownpestel")
    }
  })

  output$downpestel <- downloadHandler(
    filename =  function() {
      paste("PESTEL-",Sys.Date(),input$butdownpestel, sep=".")
    },
    # is a function with argument file. content writes the plot to the device
    content = function(file) {
      if(input$butdownpestel == "png")
        png(file) # open the png device
      else
        pdf(file) # open the pdf device
      print(values$plotpestel) # draw the plot
      dev.off()  # turn the device off

    }
  )

  output$eigenv <-  DT::renderDT({
    req(values$didybeta)
    didybeta <- values$didybeta
    names <- didybeta$term
    didybeta <- as.data.frame(didybeta)
  ...
  output$plotPestel2 <- renderPlot({
    ...
    if(input$language2 =="english"){
      tuits_afinn <-
      summar %>%
      unnest_tokens(input = "top_terms", output = "Word") %>%
      merge(values$pestel, ., by = "Word")
      values$plotpestel2 <-  tuits_afinn %>%
        count(topic, Puntuacion) %>%
        group_by(topic) %>%
        mutate(Proporcion = n / sum(n)) %>%
        ggplot() +
        aes(topic, Proporcion, fill = Puntuacion) +
        geom_col() +
        scale_y_continuous(labels = percent_format())+
        theme(legend.position = "top")
      values$t <- values$plotpestel2 + themesb[[input$themepestel]]
      values$t}
    else if(input$language2 =="spanish"){
      summar <- data.frame(topico = rownames(model$phi),
                           top_terms = top_term)
      tuits_afinn <-
        summar %>%
        unnest_tokens(input = "top_terms", output = "Palabra") %>%
        merge(values$pestel, ., by = "Palabra")
      values$plotpestel2 <-  tuits_afinn %>%
        count(topico, Puntuacion) %>%
        group_by(topico) %>%
        mutate(Proporcion = n / sum(n)) %>%
        ggplot() +
        aes(topico, Proporcion, fill = Puntuacion) +
        geom_col() +
        scale_y_continuous(labels = percent_format())+
        theme(legend.position = "top")
      values$t <- values$plotpestel2 + themesb[[input$themepestel]]
  ...
  observe({
    if (isTRUE(is.null(values$t))) {
      shinyjs::disable("downpestel2")
      shinyjs::disable("butdownpestel2")
    }
    else {shinyjs::enable("downpestel2")
      shinyjs::enable("butdownpestel2")
    }
  })

  output$downpestel2 <- downloadHandler(
    filename =  function() {
      paste("PESTEL-",Sys.Date(),input$butdownpestel2, sep=".")
    },
    # is a function with argument file. content writes the plot to the device
    content = function(file) {
      if(input$butdownpestel2 == "png")
        png(file) # open the png device
      else
        pdf(file) # open the pdf device
      print(values$t) # draw the plot
      dev.off()  # turn the device off
  ...
  output$plotPestel3 <- renderPlot({
    ...
    values$tp <- purrr::map(c("political", "economic", "environmental","legal","social","technology"), function(PESTEL) {
      tuits_afinn %>%
        filter(Puntuacion ==  input$typepestel) %>%
        group_by(topic) %>%
        count(Word, sort = T) %>%
        top_n(n = 7, wt = n) %>%
        ggplot() +
        aes(Word, n, fill = topic) +
        geom_col() +
        facet_wrap("topic", scales = "free") +
        scale_y_continuous(expand = c(0, 0)) +
        coord_flip()+
        labs(title =input$typepestel)

  })
    values$tp <- values$tp + themesb[[input$themepestel]]
    values$tp
    } else if(input$language2 == "spanish"){
      tuits_afinn <-
        summar %>%
 ...
 observe({
  if (isTRUE(is.null(values$tp))) {
      shinyjs::disable("downpestel3")
      shinyjs::disable("butdownpestel3")}
  else {shinyjs::enable("downpestel3")
    shinyjs::enable("butdownpestel3")}
  })

  output$downpestel3 <- downloadHandler(
    filename =  function() {
      paste("PESTEL-",Sys.Date(),input$butdownpestel3, sep=".")
    },
    # is a function with argument file. content writes the plot to the device
    content = function(file) {
      if(input$butdownpestel3 == "png")
        png(file) # open the png device
      else
        pdf(file) # open the pdf device
      print(values$tp) # draw the plot
      dev.off()  # turn the device off
